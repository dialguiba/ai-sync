...
func TestResponseTraceFlagInjectsTraceRuleIntoBaseGuidance(t *testing.T) {
	dir := t.TempDir()
	writeCanonicalSource(t, dir)

	if _, err := app.Run(dir, []string{"--response-trace"}); err != nil {
		t.Fatalf("sync with response trace failed: %v", err)
	}
...
func TestResponseTraceIsDisabledByDefaultAndRemovedOnNextSync(t *testing.T) {
	dir := t.TempDir()
	writeCanonicalSource(t, dir)

	if _, err := app.Run(dir, []string{"--target", "codex", "--response-trace"}); err != nil {
		t.Fatalf("initial sync with response trace failed: %v", err)
	}
	assertFileContains(t, filepath.Join(dir, "AGENTS.md"), "## Response Trace")

	if _, err := app.Run(dir, []string{"--target", "codex"}); err != nil {
		t.Fatalf("sync without response trace failed: %v", err)
	}
	assertFileNotContains(t, filepath.Join(dir, "AGENTS.md"), "## Response Trace")
}
